...
void Create_List(In *I){ // Create info node and empty list with head node.
    Node head; // Head: Create head node.

    if(*I != NULL) return;

    *I = malloc(sizeof(Info_Node)); // Create info node.

    head = malloc(sizeof(Each_Node));

    head->next = NULL;


    (*I)->size = 0; 
    (*I)->list = head; // Empty list.
}
...
void Destroy_List(In *I){ // Destroy info node and all nodes.
    ...
    tmp1 = (*I)->list; // Start from the beginning and head node.
...
void Print_List(In I){ // Print all list.
   ...
   if(Empty_List(I)) printf("List is empty.\n");
    ...
    tmp = I->list->next; // Start from the beginning.
...
void Insert_List(In I, Node tmp, TE elm){ // Insert node in list.
    ...
    if(tmp == NULL){ // Insert at start.

        new->next = I->list->next;
        I->list->next = new;
    }
...
Node Previous_Node_List(In I, Node tmp){ // Give previous node of list.
    ...
    if(I->list->next == tmp) return NULL;

    pre = I->list->next; // Start from the beginning.
...
void Delete_Node_List(In I, Node *tmp){ // Delete given node of list.
    ...
    if(I->list->next == *tmp){ // Delete first node.

        I->list->next = I->list->next->next;
        free(*tmp);
        *tmp = NULL;
    }
...
void Search_List(In I, Node *tmp, TE elem, int * found){
    ...
    *tmp = NULL;
    ...
    curr = I->list->next; // Start from the beginning.
