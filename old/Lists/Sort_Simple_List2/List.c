...
void Change_Node_List(In I, Node N, TE elm){ // Change element of given node.
    ...
    while(tmp){
        ...
        if(Min_Value(tmp->element,elm)) tmp = tmp->next;
    } // End while.
...
void Insert_List(In I, TE elm){ // Insert node in list.
    Node curr; // Curr: Fix nodes.
    int flag = 0;
    ...
    if(Empty_List(I)){
        ...
        while(curr != NULL && (Min_Value(curr->element,elm))){ // Cross list.
            flag++;

            if(curr->next == NULL) break;
            
             
            if(Min_Value(elm,curr->next->element)) break;
            curr = curr->next;
        } // End while.
        
        if(flag == 0) Insert_At_Start_List(I,elm); 
        
        else Insert_After_Node_List(I,curr,elm);
    }
...
void Search_List(In I, Node *tmp, TE elem, int * found){
    ...
    *tmp = NULL;
    ...
    while(curr && Min_Value(curr->element,elem)){ // Cross list.
